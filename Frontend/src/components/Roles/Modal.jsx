...
const Modal = ({ show, onClose, title, onSubmit, initialData }) => {
    const [roles, setRoles] = useState(initialData ? initialData.role : '');

    useEffect(() => {
        if (initialData) {
            setRoles(initialData.role); // Pré-remplir le champ pour la modification
        }
    }, [initialData]);
        ...
        >
            ...
            >
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        <label className="block text-gray-700" htmlFor="province">Roles</label>
                        <input
                            type="text"
                            id="role"
                            value={roles}
